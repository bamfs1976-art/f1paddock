...
export default function WeatherBar({ weather, compact }: Props) {
  if (!weather) {
    return (
      <div className={`label-mono text-ink-3 ${compact ? '' : 'border border-rule px-3 py-2'}`}>
        WEATHER DATA UNAVAILABLE
      </div>
    );
  }
  return (
    <div className={`flex flex-wrap items-center gap-4 font-mono text-xs ${compact ? '' : 'border border-rule bg-paper-2 px-4 py-2'}`} aria-label="Track weather">
      ...
      {weather.rainfall > 0 && (
        <span className="flex items-center gap-1.5 text-ink"><CloudRain size={12} aria-hidden="true" /> RAIN</span>
      )}
    </div>
  );
}
